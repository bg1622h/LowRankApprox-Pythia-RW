...
from StochasticGaLore2Projector import StochasticGaLore2Projector
from SVD import get_svd


class AdaptiveStochasticGaLore2Projector(StochasticGaLore2Projector):
    """Layer-wise adaptive-rank variant of the randomized-SVD stochastic sampler.
    ...
    sampler implemented by ``StochasticGaLore2Projector``.
    """

    projector_name = "adaptive_stochastic_galore2"
    metrics_prefix = "adaptive_stochastic_galore2"
        ...
        **kwargs,
        ...
        if experiment is not None and param_name is not None and step is not None:
            ...
            experiment.log_metrics(
                {
                    f"{prefix}/{param_name}/rank": self.last_adaptive_rank,
                    f"{prefix}/{param_name}/temperature": self.last_temperature,
                    f"{prefix}/{param_name}/candidate_count": self.last_candidate_count,
                },
                step=step,
            )
